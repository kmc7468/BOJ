...
int main() {
	...
	std::size_t i = 0;
	AddSub(expression, i, root);
	RemoveParens(root, &tree, i = 0);
...
void RemoveParens(Node* node, Node* parent, std::size_t& index) {
	if (node->Type == NODE_PAREN) {
		...
		if (child->Type == NODE_CHAR || parent->Type == NODE_ADDSUB && child->Type == NODE_MULDIV) {
			...
			if (IsNegativeConnecter(connecter)) {
				for (std::size_t i = 1; i < child->Children.size(); ++i) {
					child->Children[i]->Connecter = GetNegativeConnecter(child->Children[i]->Connecter);
				}
			}
